...
// Exercice 3 : Masquer les produits hors stock
$conf->global->STOCK_HIDE_OUT_OF_STOCK = 1;
/** 
 * @var Conf $conf
 * @var DoliDB $db
...
if (empty($reshook)) {
	...
	if ($action == 'confirm_clone' && $confirm == 'yes' && $usercancreate) {
		...
		// Create proposal
		$object->socid = $socid;
		// Auto-remplissage forcé du champ Réf. client
$object->socid = $socid;

if (!empty($object->socid)) {
    require_once DOL_DOCUMENT_ROOT . '/societe/class/societe.class.php';
    $soc_static = new Societe($db);
    if ($soc_static->fetch($object->socid) > 0) {
        $object->ref_client  = $soc_static->code_client;
        $_POST['ref_client'] = $soc_static->code_client;
    }
}
...
if ($action == 'create') {
	...
	if (empty($reshook)) {
		...
		// ==========================================
        // DEBUT AJOUT EXERCICE 1 (nassira)
        //==========================================
		// Calcul dynamique du statut d'expiration pour l'Exercice 2
if (!empty($object->fin_validite)) {
    $object->array_options['options_statut_expiration'] = ($object->fin_validite >= dol_now()) ? 'Valide' : 'Expirée';
}
