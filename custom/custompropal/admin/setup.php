...
/**
 * \file    custompropal/admin/setup.php
 * \ingroup custompropal
 * \brief   Custompropal setup page.
 */
...
require_once '../lib/custompropal.lib.php';
...
// Translations
$langs->loadLangs(array("admin", "custompropal@custompropal"));

// Initialize a technical object to manage hooks of page. Note that conf->hooks_modules contains an array of hook context
/** @var HookManager $hookmanager */
$hookmanager->initHooks(array('custompropalsetup', 'globalsetup'));
...
$item = $formSetup->newItem('CUSTOMPROPAL_MYPARAM1');
$item->fieldParams['isMandatory'] = 1;
$item->fieldAttr['placeholder'] = (empty($_SERVER['HTTPS']) ? 'http://' : 'https://') . $_SERVER['HTTP_HOST'];
$item->cssClass = 'minwidth500';

// Setup conf for selection of a simple string input
$item = $formSetup->newItem('CUSTOMPROPAL_MYPARAM2');
$item->defaultFieldValue = 'default value';
$item->fieldAttr['placeholder'] = 'A placeholder here';
$item->helpText = 'Tooltip text';

// Setup conf for selection of a simple textarea input but we replace the text of field title
$item = $formSetup->newItem('CUSTOMPROPAL_MYPARAM3');
$item->nameText = $item->getNameText().' more html text ';

// Setup conf for a selection of a Thirdparty
$item = $formSetup->newItem('CUSTOMPROPAL_MYPARAM4');
$item->setAsThirdpartyType();

// Setup conf for a selection of a boolean
$formSetup->newItem('CUSTOMPROPAL_MYPARAM5')->setAsYesNo();	 // ->fieldParams['alertifoff'] = 1 or ->fieldParams['alertifon'] = 1;

// Setup conf for a selection of an Email template of type thirdparty
$formSetup->newItem('CUSTOMPROPAL_MYPARAM6')->setAsEmailTemplate('thirdparty');

// Setup conf for a selection of a secured key
//$formSetup->newItem('CUSTOMPROPAL_MYPARAM7')->setAsSecureKey();

// Setup conf for a selection of a Product
$formSetup->newItem('CUSTOMPROPAL_MYPARAM8')->setAsProduct();
...
$formSetup->newItem('CUSTOMPROPAL_MYPARAM9')->setAsSelect($TField);

// Setup conf for a multiselect combo list
$item = $formSetup->newItem('CUSTOMPROPAL_MYPARAM10');
$item->setAsMultiSelect($TField);
$item->helpText = $langs->transnoentities('CUSTOMPROPAL_MYPARAM10');

// Setup conf for a category selection
$formSetup->newItem('CUSTOMPROPAL_CATEGORY_ID_XXX')->setAsCategory('product');

// Setup conf CUSTOMPROPAL_MYPARAM10
$item = $formSetup->newItem('CUSTOMPROPAL_MYPARAM10');
$item->setAsColor();
...
$item = $formSetup->newItem('CUSTOMPROPAL_MYPARAM11')->setAsHtml();
$item->nameText = $item->getNameText().' more html text ';
$item->fieldInputOverride = '';
$item->helpText = $langs->transnoentities('HelpMessage');
$item->cssClass = 'minwidth500';

$item = $formSetup->newItem('CUSTOMPROPAL_MYPARAM12');
$item->fieldOverride = "Value forced, can't be modified";
$item->cssClass = 'minwidth500';

//$item = $formSetup->newItem('CUSTOMPROPAL_MYPARAM13')->setAsDate();	// Not yet implemented
...
if ($action == 'updateMask') {
	...
	foreach ($dirmodels as $reldir) {
		$file = dol_buildpath($reldir."core/modules/custompropal/doc/pdf_".$modele."_".strtolower($tmpobjectkey).".modules.php", 0);
		if (file_exists($file)) {
	...
	if ($className !== '') {
		...
		if ($module->write_file($tmpobject, $langs) > 0) {
			header("Location: ".DOL_URL_ROOT."/document.php?modulepart=custompropal-".strtolower($tmpobjectkey)."&file=SPECIMEN.pdf");
			exit;
		} else {
	...
	if (!empty($tmpobjectkey)) {
		$constforval = 'CUSTOMPROPAL_'.strtoupper($tmpobjectkey)."_ADDON";
	...
	if ($ret > 0) {
		if (!empty($tmpobjectkey)) {
			$constforval = 'CUSTOMPROPAL_'.strtoupper($tmpobjectkey).'_ADDON_PDF';
...
$help_url = '';
$title = "CustompropalSetup";

llxHeader('', $langs->trans($title), $help_url, '', 0, 0, '', '', '', 'mod-custompropal page-admin');
...
$head = custompropalAdminPrepareHead();
print dol_get_fiche_head($head, 'settings', $langs->trans($title), -1, "custompropal@custompropal");

// Setup page goes here
echo '<span class="opacitymedium">'.$langs->trans("CustompropalSetupPage").'</span><br><br>';
	...
	foreach ($refGenerationObjects as $myTmpObjectKey => $myTmpObjectArray) {
		...
		foreach ($dirmodels as $reldir) {
			...
			while (($file = readdir($handle)) !== false) {
				...
				$constforvar = 'CUSTOMPROPAL_'.strtoupper($myTmpObjectKey).'_ADDON';
		...
		foreach ($dirmodels as $reldir) {
			foreach (array('', '/doc') as $valdir) {
				...
				foreach ($filelist as $file) {
					...
					print '<td class="center">';
					$constforvar = 'CUSTOMPROPAL_'.strtoupper($myTmpObjectKey).'_ADDON_PDF';
